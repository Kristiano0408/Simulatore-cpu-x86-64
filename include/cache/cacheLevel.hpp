...
class CacheLevel: public TickedDevice, public FaultDevice
{
    private:
        ...
        public:
        CacheLevel(uint32_t size, uint8_t associativity, uint8_t latency, uint8_t fillLatency, Bus& bus, CacheLevel* nextLevel = nullptr, CacheLevel* parentLevel1 = nullptr, CacheLevel* parentLevel2 = nullptr, CacheLevelType type = CacheLevelType::NONE, std::array<CacheLevel*, 2> L1Icourse = {nullptr, nullptr}, std::array<CacheLevel*, 2> L1Dcourse = {nullptr, nullptr}); // Constructor to initialize the cache level with size, associativity, latency, fill latency, bus reference, next level reference, parent level references, cache level type and L1 courses
        ~CacheLevel() = default;
        ...
        void fillToL1Idouble(CacheLine* line1, CacheLine* line2, const AddressInfo& addressInfo1, const AddressInfo& addressInfo2); // Function to fill a cache line in parents with L1I data for cross lines
        void fillToL1Ddouble(CacheLine* line1, CacheLine* line2, const AddressInfo& addressInfo1, const AddressInfo& addressInfo2); // Function to fill a cache line in parents with L1D data for cross lines
        ...
        void setL1Ipath(std::array<CacheLevel*, 2>&& path) { L1Ipath = std::move(path); }
        const std::array<CacheLevel*, 2>& getL1Ipath() const { return L1Ipath; }

        void setL1Dpath(std::array<CacheLevel*, 2>&& path) { L1Dpath = std::move(path); }
