...
void OperandEngine::sendReadRequest(Instruction* instruction, Operand* srcOperand, Operand* destOperand, void* callbackContext, void (*callback)(void* context))
{
    readQueue.push({.instruction = instruction, .srcOperand = srcOperand, .destOperand = destOperand, .callbackContext = callbackContext, .callback = callback});
}

void OperandEngine::sendWriteRequest(Instruction* instruction, Operand* operand, uint64_t value, void* callbackContext, void (*callback)(void* context))
{
    writeQueue.push({.instruction = instruction, .operand = operand, .value = value, .callbackContext = callbackContext, .callback = callback});
}
...
void OperandEngine::execute_operation()
{
    ...
    if (!writeQueue.empty())
    {
        ...
        writeOperand(context.instruction, context.operand, context.value, context.callbackContext, context.callback);
    }
    ...
    if (!readQueue.empty())
    {
        ...
        {
            readOperand(context.instruction, context.srcOperand, WhichOperand::SOURCE, context.callbackContext, context.callback);
        }
        if (!tempValues.isDestValueReady)
        {
            readOperand(context.instruction, context.destOperand, WhichOperand::DESTINATION, context.callbackContext, context.callback);
        }
    }
}
...
#define readValue(type, getValueFunction, resultVariable)                                                                                                                          \
    ...
    if ((resultVariable).status == OperandStatus::OK && instruction->getTemporaryValuesRef().isSrcValueReady && instruction->getTemporaryValuesRef().isDestValueReady)             \
    {                                                                                                                                                                              \
        DEBUG_LOG(debugLog("OperandEngine: " + std::string(type) + " read completed for instruction with ID " + std::to_string(instruction->getCore().InstructionId)));            \
...
void OperandEngine::readOperand(Instruction* instruction, Operand* operand, WhichOperand  whichoperand, void* callbackContext, void (*callback)(void* context))
{
    DEBUG_LOG(debugLog("OperandEngine: Reading operand of type " + std::to_string(static_cast<uint8_t>(operand->getType())) + " for instruction with ID " +
    ...
    switch (operand->getType())
    {
        ...
        case OperandType::MEMORY:
            callback(callbackContext); // Call the callback function to initiate memory access(set teh corrisponding stage to WAITING_MEMORY)
            readValue("memory",

                      getMemoryValue(instruction, operand, executeEngineEventHandler.getContext(),
                                     executeEngineEventHandler.getCallback(EventHandlerExecuteEngineEventType::MEMORY_DONE), whichoperand),
                      result);
            return;
...
#define writeValue(type, setValueFunction, resultVariable, callbackType)                                                                                                           \
    ...
    {                                                                                                                                                                              \
        DEBUG_LOG(debugLog("OperandEngine: " + std::string(type) + " write completed for instruction with ID " + std::to_string(instruction->getCore().InstructionId)));           \
...
void OperandEngine::writeOperand(Instruction* instruction, Operand* operand, uint64_t value, void* callbackContext, void (*callback)(void* context))
{
    DEBUG_LOG(debugLog("OperandEngine: Writing value " + std::to_string(value) + " to operand of type " + std::to_string(static_cast<uint8_t>(operand->getType())) +
                       " for instruction with ID " + std::to_string(instruction->getCore().InstructionId)));
    OperandResult result;
    switch (operand->getType())
    {
        case OperandType::REGISTER:
            writeValue("register", setRegisterValue(operand, value), result, EventHandlerExecuteEngineEventType::OPERAND_COMPLETE_WRITE_WRITEBACK);
            return;
        case OperandType::MEMORY:
            callback(callbackContext); // Call the callback function to initiate memory access(set teh corrisponding stage to WAITING_MEMORY)
            writeValue("memory",
...
OperandResult OperandEngine::getMemoryValue(Instruction* instruction, Operand* operand, void* callbackContext, void (*callback)(void* context), WhichOperand whichoperand)
{
    ...
    if (!memOperand->isReadRequestSent())
    {
        ...
        CacheRequest request(RequestType::READ, dataTypeSize, dataMemoryInterface.type, memOperand->getAddress(), MaxCPUInstructionLength{}, false, core.InstructionId,
