...
ExecuteEngine::ExecuteEngine(CPU& cpuRef, RegisterFile& regFile, ALU& aLu, CacheManager& cManager, PipelineEventHandler* pEventHandler)
    : cpu(cpuRef), registerFile(regFile), alu(aLu), cacheManager(cManager), operandEngine(cManager, cpuRef, executeEngineEventHandler), pipelineEventHandler(pEventHandler)
{
...
void ExecuteEngine::execute_operation()
{
    ...
    {
        processQueue(memoryAccessQueue, requestMemoryAccess);
    }

    if (!executionQueue.empty())
    {
        processQueue(executionQueue, startExecution);
    }
...
void ExecuteEngine::startExecution(Instruction* instruction)
{
    if (isExecuting)
    {
        DEBUG_LOG(debugLog("ExecuteEngine is already executing an instruction. Cannot execute another instruction simultaneously with ID " +
    ...
    uint8_t bit = instruction->calculatingNumberOfBits();
