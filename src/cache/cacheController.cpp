...
AddressInfo CacheController::decodeAddress(uint64_t address)
{
    ...
    auto setIndexBits = (uint8_t)std::countr_zero(numSets);       // Calculate the number of bits for the set index based on the number of cache sets

    uint64_t offset = address & ((1ULL << offsetBits) - 1);                             // Extract the offset from the address using bitwise operations
    auto setIndex = (uint32_t)((address >> offsetBits) & ((1ULL << setIndexBits) - 1)); // Extract the set index from the address using bitwise operations
    ...
    return {address, setIndex, tag, offset}; // Return an AddressInfo struct containing the decoded address information
}
...
void CacheController::handleRequest(PendingRequest& request)
{
    ...
    if (cacheRequest.type == RequestType::FILL) // If the request is a FILL request and it's a hit, we need to fill the cache line with data from the next level or memory
    {
        ...
        CacheEventPayload payloadFill =
            CacheEventPayload(addressInfo, cacheRequest, &(request.line)); // Create a payload for the fill event with the address information and cache request
    ...
    if (cacheRequest.type == RequestType::READ_AFTER_FILL || cacheRequest.type == RequestType::WRITE_AFTER_FILL)
    {
    ...
    switch (result)
    {
        ...
        case LookUpResult::HIT_CROSS_LINES:
            ...
            payload.addressInfo2 =
                decodeAddress(cacheRequest.address + (CACHE_LINE_SIZE - addressInfo.offset)); // Decode the address of the second cache line for cross-line access
            cacheEventHandler.triggerHandleRequestEvent(EventHandlerCacheEventType::CACHE_HIT_CROSS_LINES,
                                                        payload); // Trigger a cache hit cross lines event for logging or debugging purposes
            break;
...
LookUpResult CacheController::lookupCache(const AddressInfo& addressInfo, TypeofData dataType)
{
    ...
    CacheLookupPayload payload =
        CacheLookupPayload(addressInfo, dataType);               // Create a payload for the cache lookup event with the address information and type of data being accessed
